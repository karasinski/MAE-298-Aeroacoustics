...
\end{align*}
Similarly, the velocity normal to the vortex sheet must be equivalent on both sides of the sheet (i.e., $v_r^+ = v_r^-$). Considering the momentum equation in the normal direction
\begin{align*}
\rho_0 \dfrac{D \vec{v}}{D t} &= - \nabla P \\
\rho_0 \dfrac{D \vec{v_r}}{D t} &= - \nabla P \cdot \hat{n} = -\dfrac{\partial P}{\partial r}
\end{align*}
and
\begin{align*}
\vec{v} \cdot \hat{n} = \vec{v_r} = \hat{v_r} e^{-i w t + k z}
\end{align*}
Expanding the total derivative
\begin{align*}
\rho_0 \left( \dfrac{\partial}{\partial t} + W \dfrac{\partial}{\partial z} \right) \vec{v_r} &= -\dfrac{\partial P}{\partial r}
\end{align*}
We can define the position of the vortex sheet, $\zeta(z, t)$, as
\begin{align*}
\zeta(z, t) &= a e^{-i w t + k z} \\
\hat{v_r} &= \dfrac{d \zeta}{d t}
\end{align*}
We can find the normal velocity on the outside ($\hat{v_r}^+$) and inside ($\hat{v_r}^-$) the jet
\begin{align*}
\hat{v_r}^+ &= \left( \dfrac{\partial}{\partial t} + W \dfrac{\partial}{\partial z} \right) \zeta_{\infty} \\
&= -i a \left( w - W_{\infty} k \right) \zeta \\
\hat{v_r}^- &= \left( \dfrac{\partial}{\partial t} + W \dfrac{\partial}{\partial z} \right) \zeta_{j} \\
&= -i a \left( w - W_{j} k \right) \zeta
\end{align*}
