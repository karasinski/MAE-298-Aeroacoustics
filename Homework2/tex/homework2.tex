...
\\
The velocity normal to the vortex sheet must be equivalent on both sides of the sheet (i.e., $v_r^+ = v_r^-$). Considering the momentum equation
\begin{align*}
\overline{\rho_0} \dfrac{D \vec{v}}{D t} &= - \nabla P
\end{align*}
We define ${v_r} = v e^{-i (w t + k z)}$, where $v$ is an arbitrary constant, and dot both sides normal to the radial direction
\begin{align*}
\overline{\rho_0} \dfrac{D}{D t} \left ( \vec{v} \cdot \hat{r} \right ) &= - \nabla P \cdot \hat{r} \\
\overline{\rho_0} \left( \dfrac{\partial}{\partial t} + W \dfrac{\partial}{\partial z} \right) {v_r} &= -\dfrac{d P}{d r} \\
...
i \overline{\rho_0} \left(w - Wk \right) {v_r} &= \dfrac{d P}{d r}
\end{align*}
Rearranging, and solving for both inside the jet and outside in the freestream, we find
\begin{align}
{v_r}^+ &= \dfrac{\frac{d P}{d r}^+} {i \overline{\rho_0} \left(w - W_{\infty}k \right) } \\
{v_r}^- &= \dfrac{\frac{d P}{d r}^-}{i \overline{\rho_0} \left(w - W_{j}k \right) }
\end{align}
We can define the position of the vortex sheet, $\zeta = \zeta^+ = \zeta^- = \zeta(z, t) = a e^{-i (w t + k z)}$, and
\begin{align*}
{v_r} &= \dfrac{D }{D t} \zeta(z, t) \\
